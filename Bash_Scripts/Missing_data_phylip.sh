#!/bin/bash
#Designed for unmodified phylip outputs from STACKS
#Usuage: Missing_data_phylip.sh file.phylip

file=${1?Error: Please Provide Phylip File}

#Count the # of sites for each pop
	total_count=$(tail -n+2 $file | grep -v '#' | cut -f2- | awk '{print length}')

#Count the # of N's for each pop
	count=$(tail -n+2 $file | grep -v '#' | cut -f2 | awk '{print gsub("N","")}')
